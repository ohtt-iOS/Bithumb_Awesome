...
private func toKRWPrice(btcPrice: String?) -> String {
  ...
  guard let number = numberFormatter.string(from: NSNumber(value: round(btc * doubleValue * 1000) / 1000))
  else {
    return ""
  }
  return number + "백만"
...
private func setformat(of tradeValue: String?) -> String {
  ...
  if doubleValue > unit {
    guard let number = numberFormatter.string(from: NSNumber(value: Int(doubleValue / unit)))
  else {
    return ""
  }
    return number + "백만"
...
private func toKRWTradeValue(of tradeValue: String?) -> String {
  ...
  let btc: Double = 50000000
  let unit: Double = 1000000
  guard let number = numberFormatter.string(from: NSNumber(value: Int(btc * doubleValue / unit)))
  else {
    return ""
  }
  return number + "백만"
